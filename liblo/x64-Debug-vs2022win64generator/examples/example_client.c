...
//    lo_address t = lo_address_new_from_url( "osc.unix://localhost/tmp/mysocket" );
    lo_address t = lo_address_new(host, server_port);
    printf("sending to server in host: %s\n", lo_address_get_hostname(t));
    printf("sending to server with port: %s \n-------------------------\n\n", lo_address_get_port(t));
    ...
    if (argc > 1 && argv[1][0] == '-' && argv[1][1] == 'q') {
        ...
        lo_send(t, "/ApriltagPipeIn/testing", "s", "Hi From example client");
