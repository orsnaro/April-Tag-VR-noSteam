 ...
 */
 ...
 //ORS note on LO_CHECK_BEFORE : how it calles is_valid() in global SCOPE while is_valid() functions all are defined in class? 
 // ans: cuz the macor sees the inner most scope of where it's been called from i.e.(MACROS sees call scope first not their declare/define scope)
...
#define LO_CHECK_AFTER
    ...
    class Address
    {
      public:
        ...
        std::string url() const
        {
            ...
            //free(s); //dont free non dynamically alocated variable!
            return result;
        }
    ...
    class Server
    {
      public:
        ...
        ///ORSNARO ADD NEW CONSTRUCTOR
        /** Constructor taking a port number, protocol, ip and error handler. */
            Server(const num_string_type& port, int proto, const string_type& ip = "", lo_err_handler err_h = 0)
            : Server(lo_server_new_with_proto(port, proto, err_h))
            { LO_CHECK_AFTER; }
        ...
        std::string url() const
        {
            ...
            std::string result(s?s:"");
            //free(s); //dont free non dynamically alocated varialble
            return result;
        }
    ...
    class ServerThread : public Server
    {
      public:
        ...
        { if (iface._s || ip._s)
         ...
         ServerThread(const num_string_type &port, const string_type &ip) : Server(0)
        { if (ip._s)
            server_thread = lo_server_thread_new_multicast_iface(NULL, port,
                                                                 NULL, ip, NULL);
          else
            server_thread = lo_server_thread_new_multicast(NULL, port, NULL);
          if (server_thread)
            server = lo_server_thread_get_server(server_thread);
          LO_CHECK_AFTER;
        }
